...
# ==============================================================================
# Single entrypoint for OpenTofu infrastructure + post-provision Azure CLI
# reconciliation.
#
...
# (serving app and training job) are created by this script via Azure CLI
# after the environment is fully provisioned.  This avoids the ARM
# "Operation expired" timeout that occurs when Terraform provisions
# Container Apps and their environment in the same plan on student
# subscriptions.
#
...
# Always run from the script's own directory so relative paths are stable
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd -P)"
cd "$SCRIPT_DIR" || { echo "ERROR: cannot cd to $SCRIPT_DIR" >&2; exit 1; }

# Generated by bootstrap.sh – contains backend storage config and the
# subscription suffix that every resource name depends on.
...
# ===========================================================================
# 1. Auth mode – TF_BACKEND_AUTH_MODE is the single source of truth.
#    Supported values: oidc | cli | access_key
# ===========================================================================
...
# ===========================================================================
# 2. Install OpenTofu if missing (pinned version)
# ===========================================================================
...
# ===========================================================================
# 3. Resolve Azure context – subscription and tenant are auto-detected
#    from the current az CLI login unless TF_VAR_* is already set.
# ===========================================================================
...
# ===========================================================================
# 4. Backend authentication – builds the azurerm backend config
#    that points at the state storage account created by bootstrap.
# ===========================================================================
choose_auth_mode() {
...
compute_defaults() {
  local s="${SUBSCRIPTION_SUFFIX:-${TF_VAR_subscription_id: -6}}"
...
init_backend() {
  local cfg; cfg="$(build_backend_config)"
...
#    Every resource name is built from subscription suffix + environment.
# ===========================================================================
derive_names() {
  local s="${TF_VAR_subscription_id: -6}"
  local p="sm"
  local e
  case "$ENVIRONMENT" in
    staging) e="stg" ;;
...
# ===========================================================================
# 7. Safely read a Terraform output (returns empty string if not found)
# ===========================================================================
get_tofu_output() { tofu output -raw "$1" 2>/dev/null || true; }

# ===========================================================================
# 8. Idempotent role assignment – only creates the role if it doesn't exist
# ===========================================================================
...
# ===========================================================================
# 9. Serving app – created and healed entirely by CLI.
#    Avoids the ARM "Operation expired" timeout on student subscriptions.
# ===========================================================================
create_serving_app() {
  derive_names
  ...
  local conn_str="${APP_INSIGHTS_CONNECTION_STRING:-$(get_tofu_output application_insights_connection_string)}"
  ...
  # Create the app asynchronously with system-assigned managed identity.
  # The live CLI spelling is --revisions-mode, and env vars must be passed as
  ...
  # Poll until the app reaches Succeeded state (up to 30 minutes)
...
# ===========================================================================
# 10. Training job management – created and healed entirely by CLI.
#     RBAC roles are assigned after the job identity exists.
# ===========================================================================
...
assign_training_job_rbac() {
  ...
  local store="/subscriptions/${TF_VAR_subscription_id}/resourceGroups/${RG_NAME}/providers/Microsoft.Storage/storageAccounts/${STORAGE_ACCOUNT_NAME}"
  local acr="/subscriptions/${TF_VAR_subscription_id}/resourceGroups/${RG_NAME}/providers/Microsoft.ContainerRegistry/registries/${ACR_NAME}"
...
assign_function_operator_role() {
  ...
  local job_scope="/subscriptions/${TF_VAR_subscription_id}/resourceGroups/${RG_NAME}/providers/Microsoft.App/jobs/${TRAIN_JOB_NAME}"
...
create_training_job() {
  ...
  # Read workspace details from Terraform outputs
  ...
  # Create the job asynchronously, then poll until ready
...
# ===========================================================================
# 11. Nuclear destroy – deletes the resource group, purges soft-deleted
#     Key Vault and ML workspace, and removes the Terraform state blob.
# ===========================================================================
nuclear_destroy() {
  log "starting nuclear destroy for $ENVIRONMENT"
  derive_names
  local sub="${TF_VAR_subscription_id: -6}" loc="eastus2"
  local rg="rg-sm-artifacts-${ENVIRONMENT:0:3}"
  case "$ENVIRONMENT" in staging) rg="$RG_NAME" ;; prod) rg="rg-sm-artifacts-prod" ;; esac
  ...
  local kv="kv-sm${ENVIRONMENT:0:3}ml${sub}"
  local ml="mlw-sm-${ENVIRONMENT:0:3}"
  log "purging soft-deleted Key Vault ${kv} and ML workspace ${ml}"
  az keyvault purge -n "$kv" 2>/dev/null || true
  az rest --method delete \
    --url "https://management.azure.com/subscriptions/${TF_VAR_subscription_id}/providers/Microsoft.MachineLearningServices/locations/${loc}/deletedWorkspaces/${ml}?api-version=2024-10-01" \
    2>/dev/null || true
...
# ===========================================================================
# 12. Deploy the Azure Function blob-trigger code using remote build
# ===========================================================================
deploy_function_code() {
  ...
  # Install Azure Functions Core Tools v4 (pinned) if not already present
  ...
  # Remote build – Azure installs dependencies from requirements.txt
...
# ===========================================================================
# 14. Auto-derive Azure DevOps variables from subscription and git remote
# ===========================================================================
resolve_git_remote() {
  command -v git >/dev/null 2>&1 || return 1
  local u; u="$(git -C "$SCRIPT_DIR" remote get-url origin 2>/dev/null || true)"
  [[ -n "$u" ]] || return 1
  case "$u" in
...
# ===========================================================================
# 15. Execution – dispatch to the requested mode
# ===========================================================================
load_bootstrap_env; resolve_azure_context; choose_auth_mode; install_tofu_if_needed
compute_defaults; resolve_ado_vars

# Map Azure DevOps credentials to the environment variables run.sh expects
...
case "$MODE" in
  ...
  --create)
    ...
    # Apply the full Terraform plan. If it fails (e.g. student subscription
    # timeout), log the error but continue – the CLI-based healing steps below
    # will fix any failed resources.
    tofu apply -input=false -lock-timeout=5m -auto-approve "$PLAN_FILE" || {
      log "Terraform apply had errors – continuing with CLI healing"
    }

    # Read Terraform outputs needed by the CLI-managed resources
    ...
    # Create or heal the serving app (CLI, no Terraform timeouts)
    log "creating serving app"
    create_serving_app

    # Create or heal the training job (CLI, no Terraform timeouts)
    log "ensuring training job exists"
    ensure_job_provisioned

    # Grant the Function App permission to start the training job
